...
def sauvola(img, window, thresh, k):
    ...
    isum = sum2[window[0]:rows + window[0], window[1]:cols + window[1]] +         sum2[0:rows, 0:cols] -         sum2[window[0]:rows + window[0], 0:cols] -         sum2[0:rows, window[1]:cols + window[1]]

    isqsum = sqsum[window[0]:rows + window[0], window[1]:cols + window[1]] +         sqsum[0:rows, 0:cols] -         sqsum[window[0]:rows + window[0], 0:cols] -         sqsum[0:rows, window[1]:cols + window[1]]
...
#().run_line_magic('cd', "'/home/kuadmin01/terng/'")
#().system('ls')
...
#().run_line_magic('cd', 'SeniorProjectMaterial/')
EPOCHS = 100
...
#model.load_weights(f"target/Flor/{INPUT_SOURCE_NAME}_checkpoint_weights.hdf5")


# In[14]:


#().system('mkdir -p saved_model')
model.save(f"saved_model/Flor/{INPUT_SOURCE_NAME}_filter")




converter = tf.lite.TFLiteConverter.from_saved_model(f"saved_model/Flor/{INPUT_SOURCE_NAME}_filter")

converter.experimental_new_converter = True
tflite_model = converter.convert()
open(f"{INPUT_SOURCE_NAME}_Flor.tflite", "wb").write(tflite_model)
